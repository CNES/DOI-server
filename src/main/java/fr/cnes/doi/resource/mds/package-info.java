 ...
 * <ul>
 * <li>1. Create a project on the server by using the {@link fr.cnes.doi.application.AdminApplication}
 * through the resource {@link fr.cnes.doi.resource.admin.SuffixProjectsResource#createProject}.
 * The response at this resource is an identifier, which is a part of the DOI suffix. This suffix
 * guarantees the uniqueness of the DOI within the institution at condition that the project defines
 * an identifier for each data. This data identifier will be part of the DOI suffix. Thus, the new
 * DOI suffix has the following syntax <b>projectIdentifier/dataIdentifier</b></li>
 * <li>2 - Create the DOI metadata according to the {@value fr.cnes.doi.client.ClientMDS#SCHEMA_DATACITE}
 * schema.</li>
 * <li>3 - Upload the DOI metadata by using the {@link fr.cnes.doi.application.DoiMdsApplication} application
 * through the {@link fr.cnes.doi.resource.mds.MetadatasResource}</li>
 * <li>4 - Upload the landing page URL using {@link fr.cnes.doi.application.DoiMdsApplication} application
 * through the {@link fr.cnes.doi.resource.mds.DoisResource}</li>
 * </ul>
