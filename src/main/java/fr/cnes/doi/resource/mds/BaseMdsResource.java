...
public class BaseMdsResource extends AbstractResource {
    ...
    /**
     * Returns the suffix project with which the user is associated. For each suffix
     * project, a role is associated. To get the suffix projet, the role must be get
     * in the HTTP header. When several roles are associated to a user, a
     * <i>selectedRole</i> must be provided to select the role that the user wants
     * to get.
     *
     * @param selectedRole
     *            selected role
     ...
     * @throws DoiServerException
     *             - CLIENT_ERROR_FORBIDDEN if the role is not allowed to use this
     *             feature - CLIENT_ERROR_UNAUTHORIZED if no role is provided -
     *             CLIENT_ERROR_CONFLICT if a user is associated to more than one
     *             role
     */
	...
	if (hasNoRole(roles)) {
	    ...
	    throw LOG.throwing(Level.DEBUG, new DoiServerException(getApplication(),
		    API_MDS.SECURITY_USER_NO_ROLE, "User not contained in any role"));
	    ...
	    throw LOG.throwing(Level.DEBUG, new DoiServerException(getApplication(),
		    API_MDS.SECURITY_USER_CONFLICT, "Cannot know which role must be applied"));
	}
    }
    ...
    /**
     * Checks permissions to access according to the role of the user and the DOI
     * name. The authorization in DOI server is based on <b>role</b>. Each
     * projectSuffix is
     * <ul>
     * <li>a <b>role</b></li>
     * <li>related to a projectName</li>
     * </ul>
     * When the DOI name does not start with <i>prefixDOI/role</i>, an exception is
     * send.
     *
     * @param doiName
     *            DOI name
	...
	addResponseDocToMethod(info, createResponseDoc(API_MDS.SECURITY_USER_NO_ROLE.getStatus(),
		API_MDS.SECURITY_USER_NO_ROLE.getShortMessage(), "explainRepresentationID"));
	addResponseDocToMethod(info,
		createResponseDoc(API_MDS.SECURITY_USER_NOT_IN_SELECTED_ROLE.getStatus(),
			API_MDS.SECURITY_USER_NOT_IN_SELECTED_ROLE.getShortMessage(),
			"explainRepresentationID"));
	addResponseDocToMethod(info, createResponseDoc(API_MDS.SECURITY_USER_PERMISSION.getStatus(),
		API_MDS.SECURITY_USER_PERMISSION.getShortMessage(), "explainRepresentationID"));
	addResponseDocToMethod(info, createResponseDoc(API_MDS.SECURITY_USER_CONFLICT.getStatus(),
		API_MDS.SECURITY_USER_CONFLICT.getShortMessage(), "explainRepresentationID"));
    }
	...
	addResponseDocToMethod(info, createResponseDoc(API_MDS.SECURITY_USER_NO_ROLE.getStatus(),
		API_MDS.SECURITY_USER_NO_ROLE.getShortMessage(), explainRepresentation()));
	addResponseDocToMethod(info,
		createResponseDoc(API_MDS.SECURITY_USER_NOT_IN_SELECTED_ROLE.getStatus(),
			API_MDS.SECURITY_USER_NOT_IN_SELECTED_ROLE.getShortMessage(),
			"explainRepresentationID"));
