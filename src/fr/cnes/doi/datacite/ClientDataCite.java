...
public class ClientDataCite {
     ...
     * Please use it for all your testing DOIs. Your real prefix should not be 
     * used for test DOIs. Note that DOIs with test prefix will behave like any 
     * other DOI, e.g. they can be normally resolved. They will not be exposed 
     * by upcoming services like search and OAI, though. Periodically we purge *
     * all 10.5072 datasets from the system.
     * 
     * <p>
     * It is important to understand that the Handle System 
     * (the technical infrastructure for DOIs) is a distributed network system. 
     * The consequence of this manifests is its inherent latency. For example, 
     * DOIs have TTL (time to live) defaulted to 24 hours, so your changes will 
     * be visible to the resolution infrastructure only when the TTL expires. 
     * Also, if you create a DOI and then immediately try to update its URL, 
     * you might get the error message HANDLE NOT EXISTS. This is because it 
     * takes some time for the system to register a handle for a DOI.
     * 
     * @param isTestMode Each API call can have optional query parametertestMode. 
     * If set to "true" or "1" the request will not change the database nor will
     * the DOI handle will be registered or updated, e.g. POST /doi?testMode=true 
     * and the testing prefix will be used instead of the provided prefix
     * 
     */
    public ClientDataCite(boolean isTestMode) {
        this.isTestMode = isTestMode;
        this.testMode = isTestMode ? TEST_MODE : new Parameter();
    }
     ...
     * <li>you might not be able to mint unlimited number of DOIs, there is a quota assigned to you by your allocator (the quota can be extended or lifted though)</li>
     * </ul>
     * @param isTestMode Each API call can have optional query parametertestMode.
     * If set to "true" or "1" the request will not change the database nor will
     * the DOI handle will be registered or updated, e.g. POST /doi?testMode=true.
     ...
     * @param pwd password
     */
    public ClientDataCite(boolean isTestMode, final String login, final String pwd) {
        this(isTestMode);
    ...
    private String useTestPrefix(final String doiName) {
        ...
        Engine.getLogger(ClientDataCite.class.getName()).log(Level.FINE, message);
        return testingPrefix;
    }

    /**
     * Init the client to the reference {@link DATA_CITE_URL}
     */
    private void initReference() {
        this.client.setReference(DATA_CITE_URL);
    }
    ...
    private Reference createReferenceWithDOI(final String segment, final String doiName) {        
        String requestDOI = isTestMode ? useTestPrefix(doiName) : doiName;
        Reference ref = createReference(segment);
        String[] split = requestDOI.split("/");
        for (int i=0 ;i <split.length;i++) {
            ref.addSegment(split[i]);
        }
        return ref;
    }
     ...
     * @throws ClientDataCiteException
     */
    private String getText(final Representation rep, Status status) throws ClientDataCiteException {
        String result;
        try {
            result = rep.getText();
        } catch (IOException ex) {
            throw new ClientDataCiteException(ex);
        }
        if (!status.isSuccess()) {
            throw new ClientDataCiteException(status, result);
        }
        return result;
    }

    private void checkInputForm(final Form form) {
        if (form.contains(Arrays.asList("DOI", "URL"))) {
            final String doiName = form.getFirstValue("DOI");
            checkIfAllCharsAreValid(doiName);            
            if (isTestMode) {
                form.set("DOI", useTestPrefix(doiName));
            }
        } else {
            throw new IllegalArgumentException("DOI and URL parameters are required");
        }
    }
     ...
     * @throws ClientDataCiteException An exception is thrown for a status!=2xx
     */
    public String getDoi(final String doiName) throws ClientDataCiteException {
        String result = null;        
        Reference url = createReferenceWithDOI(DOI_RESOURCE, doiName);
        Engine.getLogger(ClientDataCite.class.getName()).log(Level.FINE, "GET {0}", url.toString());
     ...
     * @throws ClientDataCiteException An exception is thrown for a status!=2xx
     */
    public String getDoiCollection() throws ClientDataCiteException {
        String result = null;
     ...
     * @throws ClientDataCiteException Will throw an exception when status!=2xx
     * @throws IllegalArgumentException Will throw this exception when DOI and
     * URL are not provided in the form
     */
    public String createDoi(final Form form) throws ClientDataCiteException {
        String result;
        this.checkInputForm(form);
        Reference url = createReference(DOI_RESOURCE);
        url = url.addQueryParameter(this.getTestMode());
        Engine.getLogger(ClientDataCite.class.getName()).log(Level.FINE, "POST {0}", url.toString());
        this.client.setReference(url);
        Representation rep = this.client.post(form);
        Status responseStatus = this.client.getStatus();
        try {
     ...
     * @throws ClientDataCiteException Will throw an exception when status !=
     * 2xx
     */
    public Resource getMetadata(final String doiName) throws ClientDataCiteException {
        Resource resource;
        ...
        Status responseStatus = this.client.getStatus();
        try {
            String result = getText(rep, responseStatus);
     ...
     */
    public String createMetadata(final Resource entity) throws ClientDataCiteException {
        String result = null;
     ...
     * @throws ClientDataCiteException
     */
    public Resource deleteMetadataDoi(final String doiName) throws ClientDataCiteException {
        Resource resource;
        Reference url = createReferenceWithDOI(METADATA_RESOURCE, doiName);
        Engine.getLogger(ClientDataCite.class.getName()).log(Level.FINE, "DELETE {0}", url.toString());
        this.client.setReference(url);
        Representation response = this.client.delete();
        Status responseStatus = this.client.getStatus();
        try {
            String result = getText(response, responseStatus);
            JAXBContext ctx = JAXBContext.newInstance(new Class[]{org.datacite.schema.kernel_4.Resource.class});
            Unmarshaller um = ctx.createUnmarshaller();
            JAXBElement<Resource> jaxbResource = (JAXBElement<Resource>) um.unmarshal(new ByteArrayInputStream(result.getBytes()));
            resource = jaxbResource.getValue();
     ...
     * @throws ClientDataCiteException Will throw an exception for status != 2xx
     */
    public String getMedia(final String doiName) throws ClientDataCiteException {
        String result;
     ...
     * 
     * Standard domain restrictions check will be performed. The different status:
     * <ul>
     * <li>200 OK - operation successful</li>
     * <li>400 Bad Request - one or more of the specified mime-types or urls are invalid (e.g. non supported mime-type, not allowed url domain, etc.)</li>
     * <li>401 Unauthorized - no login</li>
     * <li>403 Forbidden - login problem</li>
     * <li>500 Internal Server Error - server internal error, try later and if problem persists please contact us</li>    
     * </ul>
     * @param form Multiple lines in the following format{mime-type}={url} 
     * where {mime-type} and {url} have to be replaced by your mime type and URL, 
     * UFT-8 encoded.
     * @return short explanation of status code
     * @throws ClientDataCiteException 
     */
    public String createMedia(final Form form) throws ClientDataCiteException {
        String result;
        Reference url = createReference(METADATA_RESOURCE);
        Engine.getLogger(ClientDataCite.class.getName()).log(Level.FINE, "POST {0}", url.toString());        
        this.client.setReference(url);
    ...
    public static void main(String[] args) throws Exception {
        ClientDataCite client = new ClientDataCite(true);
