 ...
 * The project database stores the project name and its identifier. The
 * identifier is used in the DOI name to assure the unique name through the
 * organization. The DOI is built like this:<br>
 *
 * doi://<i>prefix organization assigned by DataCite</i>/<i>suffix project
 * assign by DOI server to a project</i>/<i>free identifier by the project</i>
 *
 *
 * <h2>Token database</h2>
 * The token database stores the created token in order to verify them when
 * there are used by the user.
 *
