 ...
 */
public final class InitServerForTest {
    ...
    /**
     * Init the settings and starts the server with a specific configuration
     * properties.
     * 
	...
	String skipMessageDisplay = (skipMessage == null) ? ""
		: ANSI_PURPLE + "  (" + skipMessage + ")" + ANSI_RESET;
	System.out.println(ANSI_BLUE + "Testing: " + title + ANSI_RESET + pt + colorStatus
		+ skipMessageDisplay);
    }

    public static void mavenInfoRun(int numberTestCases) {
	String message = "[" + ANSI_BLUE + "INFO" + ANSI_RESET + "] " + ANSI_CYAN + numberTestCases
		+ " test cases to run" + ANSI_RESET;
	System.out.println(message);
    }

    public static void mavenTitle(Failure failure) {
	String message = "[" + ANSI_BLUE + "INFO" + ANSI_RESET + "] " + ANSI_WHITE + "Running "
		+ failure.getDescription().getClassName() + ANSI_RESET;
	System.out.println(message);
    }

    public static void mavenSkipMessage(Failure failure) {
	String message = "[" + ANSI_YELLOW + "WARNING" + ANSI_RESET + "] " + ANSI_YELLOW + "Tests"
		+ ANSI_RESET + " run: " + failure.getDescription().testCount()
		+ ", Failures: 0, Errors: 0, " + ANSI_YELLOW + "Skipped: "
		+ failure.getDescription().testCount() + ANSI_RESET + ", Time elapsed: 0 s - "
		+ failure.getMessage();
	System.out.println(message);
    }
	...
	if (result.wasSuccessful()) {
	    if (skip > 0) {
		System.out.println("[" + ANSI_BLUE + "WARNING" + ANSI_RESET + "] " + ANSI_GREEN
			+ "Tests run: " + total + ", Failures: " + error + ", Skipped: " + skip
			+ ANSI_RESET);
	    } else {
		System.out.println("[" + ANSI_GREEN + "INFO" + ANSI_RESET + "] " + ANSI_GREEN
			+ "Tests run: " + total + ", Failures: " + error + ", Skipped: " + skip
			+ ANSI_RESET);
	    }
	} else {
	    System.out
		    .println("[" + ANSI_RED + "ERROR" + ANSI_RESET + "] " + ANSI_RED + "Tests run: "
			    + total + ", Failures: " + error + ", Skipped: " + skip + ANSI_RESET);
	}
    }
